  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { message: 'ERRO: A API Key do Gemini não foi encontrada! Verifique se você criou o arquivo .env.local exatamente dentro de copyzap-clone com GEMINI_API_KEY=sua_chave e reiniciou o servidor.' },
        { status: 500 }
      );
    }
...
- 20% EMPATIA (Referência: Como Fazer Amigos e Influenciar Pessoas): Compreenda as dores profundas do produtor rural (seca, custo alto da ração, frustração na colheita).
- 70% PERSUASÃO E BENEFÍCIOS (Referência: A Bíblia de Vendas e Armas da Persuasão): Foque no lucro, aumento leiteiro/corte e redução de custos. Use Autoridade e Prova Social com os números do produto.
- 10% URGÊNCIA ASSUMIDA (Referência: Lobo de Wall Street): Apenas no final do texto ou do funil, seja direto e use de técnica de fechamento absoluto para forçar a decisão (sem arrogância, apenas confiança).
...
NÃO RETORNE MARKDOWN FORA DO JSON. RETORNE SOMENTE O TEXTO JSON PURO!`;
    ...
    // Extrai apenas o bloco JSON usando Regex para evitar qualquer texto antes ou depois
    const jsonMatch = textOutput.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
        throw new Error("O formato retornado pela Inteligência Artificial foi inválido.");
    }
    ...
    console.error('Erro no Generate API:', error);
    return NextResponse.json(
